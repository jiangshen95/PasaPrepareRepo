...
class Solution3 {
    ...
    private void backtrack(char[][] board, int i, int j, TrieNode parent, List<String> result, 
        boolean[][] visited) {
        visited[i][j] = true;
        TrieNode cur = parent.chidren.get(board[i][j]);
        if(cur.word != null) {
            result.add(cur.word);
            cur.word = null;
        }
        ...
        for(int k = 0; k < 4; k++) {
            ...
            if(row >= 0 && row < board.length && col >= 0 && col < board[0].length &&
                ...
                backtrack(board, row, col, cur, result, visited);
            }
        }
        visited[i][j] = false;
        if(cur.chidren.isEmpty()) {
            parent.chidren.remove(board[i][j]);
        }
    }
}
...
public class WordSearchII3 {
    public static void main(String[] args) {
        ...
        List<String> result = new Solution3().findWords(board, words);
